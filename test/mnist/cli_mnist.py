...
def img_in_class():
    ...
    imgs = cli_train.get_stream(tw.StreamRequest(event_name='batch',
...
def show_find_lr():
    ...
    train_batch_loss = cli_train.get_stream(tw.StreamRequest(event_name='batch', 
...
def plot_grads():
    ...
    grads = train_cli.get_stream(tw.StreamRequest(event_name='batch', 
...
def plot_weight():
    ...
    params = train_cli.get_stream(tw.StreamRequest(event_name='batch', 
...
def epoch_stats():
    ...
    train_loss = train_cli.get_stream(tw.StreamRequest(event_name="epoch", 
        expr='map(lambda v:v.metrics.epoch_loss, l)'))
    plot.subscribe(train_loss, xtitle='Epoch', ytitle='Train Loss')
    
    test_acc = test_cli.get_stream(tw.StreamRequest(event_name="epoch", 
