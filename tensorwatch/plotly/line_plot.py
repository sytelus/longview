...
class LinePlot(BasePlotlyPlot):
    def __init__(self, cell=None, title=None, show_legend:bool=True, publisher_name:str=None, console_debug:bool=False,
                  is_3d:bool=False, **plot_args):
        super(LinePlot, self).__init__(cell, title, show_legend, publisher_name=publisher_name, console_debug=console_debug, **plot_args)
        self.is_3d = is_3d
    ...
    def _setup_layout(self, stream_plot):
        ...
        axis_props = BasePlotlyPlot._get_axis_common_props(stream_plot.xtitle, stream_plot.xrange)
        ...
        yaxis = 'yaxis' + (str(stream_plot.index + 1) if stream_plot.separate_yaxis else '')
        axis_props = BasePlotlyPlot._get_axis_common_props(stream_plot.ytitle, stream_plot.yrange)
        ...
        if self.is_3d:
            ...
            axis_props = BasePlotlyPlot._get_axis_common_props(stream_plot.ztitle, stream_plot.zrange)
    ...
    def clear_plot(self, stream_plot, clear_history):
        traces = range(len(stream_plot.trace_history)) if clear_history else (stream_plot.trace_index,)
        for i in traces:
